...
public void testGetPojo() throws IOException {
    HttpGet request = new HttpGet("http://localhost:8080/user/1");
    String response = this.httpClient.execute(request, new BasicResponseHandler());
    System.out.println(response);
}
```
...
```json
{"id":1,"lastName":"zxzxin","email":"123"}
```
...
```java
json = {"id" : 2, "lastName": "李四", "email" : "4545@qq.com"}
```
...
SpringBoot为我们提供了一个`RestTemplate`很方便使用。基于Http的客户端进行了改装，并且实现了与json的序列化和反序列化，非常方便。

对`RestTemplate`的简单使用:
...
在一个新的项目中访问上一个`springboot-demo`项目。
...
![assets/zx_1.png](assets/zx_1.png)
...
**就是`userservice`和`consumer-demo`两个工程，一个负责写一个简单demo，另一个项目调用这个demo**

聚合工厂结构:

![zx_2.png](assets/zx_2.png)

其中一个项目在`8081`启动，另一个在`8080`启动。
...
代码和springboot入门demo代码一样。

![zx_3.png](assets/zx_3.png)
...
- Eureka：就是**服务注册中心**（可以是一个集群），对外暴露自己的地址
...
实现三个子项目: 一个是`Eureka`注册中心，另外两个是`Eureka`客户端。一个是服务提供者`user-service`，另一个是消费者`consumer-demo`。

![zx_6.png](assets/zx_6.png)
...
spring:
  application:
    name: eureka-server # 应用名称，会在Eureka中显示, 不加就会显示那个Unknown
eureka:
  client:
...
public class UserServiceGoodApplication {
	public static void main(String[] args) {
		SpringApplication.run(UserServiceGoodApplication.class, args);
	}
}
```
...
- 删除了`register-with-eureka=false`和`fetch-registry=false`两个配置。因为默认值是true，这样就会吧自己注册到注册中心了。
